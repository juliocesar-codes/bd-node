 ...
 * npx prisma migrate reset                 -> Realiza o reset o database
...
const getSelectLastIdFilm = async function () {
    try {
        // Script SQL 
        let sql = `select id from tbl_filmes order by id desc limit 1`
...
const setInsertFilms = async function (filme) {
    try {
        let sql = `INSERT INTO tbl_filmes (nome, sinopse, data_lancamento, duracao, orcamento, trailer, capa)
        VALUES (
        '${filme.nome}',
        '${filme.sinopse}',
